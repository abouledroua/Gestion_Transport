...
class FicheActivationController extends GetxController {
  ...
  Future<bool> onWillPop() async =>
      (await showDialog(
          ...
          builder: (context) => AlertDialog(
                  title: Row(children: [
                    Icon(Icons.cancel_outlined, color: red),
                    const Padding(
  ...
  saveClient() {
    ...
    if ((saisieToken && valTokken) ||
        valNumCD ||
        (!saisieToken && (valNact1 || valNact2 || valNact3 || valNact4))) {
      if (valNumCD) {
  ...
  verifierNact() async {
    ...
    if (gNact == myNact && gNact.isNotEmpty) {
      ...
      activerDevice(
          cd: txtNumCD.text.removeAllWhitespace.toUpperCase(),
          ns: nid,
          na: myNact);
    } else {
      updateValider(newValue: false);
      AppData.mySnackBar(
          title: 'Activation',
          message: "Code d'activation éronnée !!!!",
          color: red);
    }
  }
  ...
  genererIdentifant() async {
    ...
    txtIdApp1.text = myId.substring(5, 6) +
        txtNumCD.text.substring(4, 5) +
        txtNumCD.text.substring(0, 1) +
        myId.substring(1, 2);
    txtIdApp2.text = myId.substring(0, 1) +
        txtNumCD.text.substring(2, 3) +
        myId.substring(7, 8) +
        txtNumCD.text.substring(6, 7);
    txtIdApp3.text = txtNumCD.text.substring(3, 4) +
        txtNumCD.text.substring(5, 6) +
        myId.substring(6, 7) +
        myId.substring(3, 4);
    txtIdApp4.text = txtNumCD.text.substring(1, 2) +
        myId.substring(2, 3) +
        myId.substring(4, 5) +
        myId.substring(8, 9);
    update();
